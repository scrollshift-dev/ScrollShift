...
**Current state:** the first long-running daemon and systemd service are implemented, with **automatic hotplug mouse discovery as the normal architecture**. `mode = auto` is the default: the daemon reads udev input classification (mouse/touchpad/touchscreen/joystick/tablet/pointing-stick/keyboard) when available, excludes every known non-mouse class, and falls back to a deliberately conservative evdev capability check when classification metadata is missing so a non-mouse relative device is not mistaken for a mouse. It can start with zero mice attached, waits without failing, attaches qualifying wheel mice as they appear, supports multiple simultaneous mice with independent relay workers, re-enumerates after unplug/replug or event-node renumbering, and applies bounded exponential backoff while a still-present device cannot be grabbed. `/dev/input/eventN` is never persisted. `scrollshift configure DEVICE` persists vendor/product identity plus normalized kernel device name and now serves as an unusual-hardware/troubleshooting override rather than a mandatory setup step. `scrollshift service install|uninstall|start|stop|restart|status|enable|disable|logs` owns the managed systemd lifecycle, and the unit uses `Restart=on-failure`. The current permission model is deliberately a root system service; a less-privileged udev/group model can be evaluated later rather than weakening input permissions prematurely.

**Current hardening:** automatic classification now has a deterministic classifier matrix covering conventional/high-resolution mice, touchpads, touchscreens, joysticks, drawing tablets, tablet pads, pointing sticks, keyboard consumer-control nodes, composite auxiliary nodes, trackballs, ScrollShift virtual devices, unrelated virtual pointers, and missing/partial/conflicting udev metadata. Device matching has explicit missing/unique/ambiguous states with regression coverage. Reconnect waits are interruptible so SIGTERM remains prompt even at the maximum 30-second reconnect interval. Invalid/missing configuration exits with status 2 and the systemd unit uses `RestartPreventExitStatus=2` to avoid configuration-error restart storms. `scrollshift doctor` validates configuration/device matching without grabbing input. Service ownership recognizes only the exact current unit template and exact known historical ScrollShift-generated templates; unknown marker-bearing or unrelated units are refused for install/uninstall/start/stop, and known historical units are migrated transactionally with rollback.
