...
**Release publication integrity.** Release builds use `SCROLLSHIFT_WARNINGS_AS_ERRORS=ON`; the workflow verifies the expected archive set and `SHA256SUMS` (via `scripts/verify_release.sh`) before creating a release; website installer parity is checked before publication; and an existing release is accepted on rerun only when it already contains the full expected asset set — both architecture archives **and** the published `SHA256SUMS`, with checksums matching the published archives and a manifest byte-identical to the candidate manifest. A partial release (missing `SHA256SUMS`, missing archive, malformed or differing manifest, checksum mismatch) fails loudly and is never auto-repaired. The partial-release cases have deterministic coverage in `tests/release_verify_tests.sh`. The post-publication live installer smoke test remains.
